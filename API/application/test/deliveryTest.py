...
class TestBlueprintDelivery:
    ...
    delivery = {
        "delivery":{
            ...
            "customer_id" : 12,
            "date_created" : '2015-11-30 17:23:08.321167',
            "date_pickup" : '2015-11-30 17:23:09.321167',
            "date_delivery" : '2015-11-30 17:23:10.321167'
        }
    }
...
############### CREATE #####################
    ...
    def test_create_with_wrong_customer_id(self, client, mocker):
        mocker.patch.object(DBHandler, "select", return_value={"id": 23})
    ...
    def test_create_success(self, client, mocker):
        ...
        mocker.patch.object(DBHandler, "insert", return_value={"id":34})
        res = client.post(url_for("delivery.create"), data=json.dumps(self.delivery), content_type='application/json')
        assert res.status_code == 200
        res.json = {"info": "Delivery created successfully", "deliveryId": 34}
        mocker.stopall()
